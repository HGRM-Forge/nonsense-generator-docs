...
-   **GCP API Key**: A valid Google Cloud Platform API Key with the "Cloud Natural Language API" enabled.
    -   For **Docker execution**, this key must be set as an environment variable named `GCP_API_KEY` in your shell.
...
Once the application is running, you can typically access the frontend at `http://localhost:8080`.

There are several ways to run the Nonsense Generator:

### 1. Using Docker Hub (Recommended for ease of use)

Ensure you have Docker installed and the `GCP_API_KEY` environment variable set in your shell.
The `-v "$(pwd)/buckets:/app/buckets"` command maps a directory named `buckets` in your current working directory to `/app/buckets` inside the container. This is useful if the application saves data.

```bash
# Set your GCP API Key (replace YOUR_GCP_API_KEY_HERE)
GCP_API_KEY="YOUR_GCP_API_KEY_HERE"

# Pull and run the image from Docker Hub
docker run -v "$(pwd)/buckets:/app/buckets" -e GCP_API_KEY=$GCP_API_KEY -p 8080:8080 hgrm/nonsense-generator
```
...
git clone https://github.com/HGRM-Forge/nonsense-generator.git
cd nonsense-generator

# 2. Set your GCP API Key for Docker build/run (replace YOUR_GCP_API_KEY_HERE)
GCP_API_KEY="YOUR_GCP_API_KEY_HERE"
...
# 2. Create .env file in the project root (if not already done)
echo "GCP_API_KEY=YOUR_GCP_API_KEY_HERE" > .env
